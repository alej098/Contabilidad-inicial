...
server.use((req, res, next) => {
  ...
  ); //origin es la url de donde viene la solicitud, x-requested-with es el tipo de solicitud, content-type es el tipo de contenido que se esta enviando, accept es el tipo de contenido que acepta el servidor
  res.header("Access-Control-Allow-Methods", "GET, POST, OPTIONS, PUT, DELETE"); // metodos que acepta el servidor
  next();
});

server.use("/", router); // cuando llegue una solicitud a la ruta '/' se ejecuta el archivo routes/index.js donde esatn importadas todas las rutas que creamos en la carpeta routes
